...
"""
function find_uniform_prior_mean_β(rng::Random.AbstractRNG, K::Int=50; a_σ::T = 2.5, b_σ::T = 1.5, a_τ::T = 1, b_τ::T = 1e-2, a_δ::T = 1, b_δ::T = 0.1) where {T<:AbstractFloat}
    φ = 0.95
    M = 10_000
...
function loss1(μ_k::T, p0::AbstractVector{T}, z::AbstractMatrix{T}, σ::AbsstractVector{T}) where {T<:AbstractFloat}
...
function loss2(μ_k::T, μ::AbstractVector{T}, p0::AbstractVector{T}, k::Int, z::AbstractMatrix{T}, σ::AbstractVector{T}, τ::AbstractVector{T}, δ::AbstractMatrix{T}, φ::T) where {T<:AbstractFloat}
    ...
    for m in axes(z, 1)
        ...
        θ_k[m] = BayesBSpline.stickbreaking(β)[k]
    end
