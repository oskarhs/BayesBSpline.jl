...
In general, we reccomend using the binned fitting procedure, due to the large improvements in model fitting speed, particularly for larger samples.
...
As noninformative defaults, we reccomend using `a_τ = 1`, `a_δ = 0.5`, `b_δ = 0.5`.
...
Similar models for regression suggest that values in the range [5e-5, 5e-3] are reasonable, with the default value being `b_τ = 1e-3`.
"""
