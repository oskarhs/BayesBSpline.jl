...
function compute_μ(basis::A, T::Type{<:Real}=Float64) where {A<:AbstractBSplineBasis}
    ...
    K = length(basis)
    p0 = coef_to_theta(T.(fill(1/(xmin-xmax), K)), basis)
...
function create_spline_basis_matrix_binned(x::AbstractVector{T}, basis::A, n_bins::Integer, bounds::Tuple{T,T}) where {T<:Real, A<:AbstractBSplineBasis}
    K = length(basis)
    deg = order(basis) - 1
    ...
    bincounts = BayesBSpline.bin_regular(x, bounds[1], bounds[2], n_bins, true)
