...
struct BSMChains{T<:Real, A<:AbstractBSplineBasis, NT<:NamedTuple}
    ...
    n_samples::Int
    n_burnin::Int
    function BSMChains(samples::NamedTuple{M, V}, basis::A, n_samples::Int, n_burnin::Int) where {V, M, A, NT}
        return new{eltype(V), A, NT}(samples, basis, n_samples, n_burnin)
    end
end
...
function Distributions.mean(bsmc::BSMChains, t::Real)
    bs = basis(BSMChains)
    mean_coef = mapslices(mean, bsmc.samples.coef[:, n_samples:1]; dims=2)[:]
