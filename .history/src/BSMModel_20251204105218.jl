...
function Base.show(io::IO, ::MIME"text/plain", bsm::BSMModel{T, A, NamedTuple{(:B, :b_ind, :bincounts), D}}) where {T, A, D}
...
Base.show(io::IO, bsm::BSMModel{T, S, NamedTuple{(:B, :b_ind, :bincounts), D}}) where {T, A, D} = show(io, MIME("text/plain"), bsm)
