...
function eval_posterior_splines(bsmc::BSMChains, t::AbstractVector{T}, samples_ind::AbstractVector{<:Integer}) where {T<:Real}
    bs = basis(bsmc)
    B_sparse = create_unnormalized_sparse_spline_basis_matrix(t, bs)
    coefs = bsmc.samples.coef[samples_ind, bsmc.n_burnin+1:end]
    f_samp = B_sparse * coefs
    return f_samp
end