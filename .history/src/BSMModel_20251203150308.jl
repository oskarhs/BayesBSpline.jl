...
struct BSMModel{A<:AbstractBSplineBasis, T<:Real}
    ...
    a_δ::T
    b_δ::T
    function BSMModel(basis::A; a_τ::T=1, b_τ::T=1e-3, a_δ::T=0.5, b_δ::T=0.5) where {A<:AbstractBSplineBasis, T<:Real}
        return new{A,T}(basis, a_τ::T, b_τ::T, a_δ::T, b_δ::T)
    end
end

BSMModel{T}(basis::A; kwargs...) where {A<:AbstractBSplineBasis, T<:Real} = BSMModel{T, A}(basis; kwargs...)
...
BSplineBasis.basis(bsm::B) where {B<:BSMModel} = bsm.basis
