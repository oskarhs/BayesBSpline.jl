...
struct BSMModel{T<:Real, A<:AbstractBSplineBasis, V<:AbstractVector{T}}
    ...
    function BSMModel{T,A}(x::AbstractVector{<:Real}, basis::A; n_bins::Union{Nothing,<:Integer}=1000, a_τ::Real=1, b_τ::Real=1e-3, a_δ::Real=0.5, b_δ::Real=0.5) where {T<:Real, A<:AbstractBSplineBasis}
        check_bsmkwargs(num_bins, a_τ, b_τ, a_δ, b_δ)
        new_x = T.(x)
        n = length(x)
        if !isnothing(n_bins)
        ...
        return new{T,A,typeof(new_x)}(new_x, basis, T(a_τ), T(b_τ), T(a_δ), T(b_δ))
    end
end
...
function check_bsmkwargs(n_bins::Union{Nothing,<:Integer}, a_τ::Real, b_τ::Real, a_δ::Real, b_δ::Real)
    ...
    hyperpar = [a_δ, b_δ, a_τ, b_τ]
    hyperpar_symb = [:a_δ, :b_δ, :a_τ, :b_τ]
    for i in eachindex(hyperpar)
        if hyperpar[i] ≤ 0.0
            throw(ArgumentError("Hyperparameter $(hyperpar_symb[i]) must be positive."))
        end
    end
end
