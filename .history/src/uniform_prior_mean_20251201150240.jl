...
function find_uniform_prior_mean_β(rng::Random.AbstractRNG, K::Int; a_σ::T = 2.5, b_σ::T = 1.5, a_τ::T = 1.0, b_τ::T = 1e-2, a_δ::T = 1.1, b_δ::T = 0.1, φ::T=1.0) where {T<:AbstractFloat}
    ...
    for k in 3:K-1
        ...
        res = Optim.optimize(lossfunc, -20, 1000, GoldenSection())
...
function loss1(μ_k::T, p0::AbstractVector{T}, z::AbstractMatrix{T}, σ::AbstractMatrix{T}) where {T<:AbstractFloat}
...
function loss2(μ_k::T, p0::AbstractVector{T}, z::AbstractMatrix{T}, σ::AbstractMatrix{T}, θ_cum::AbstractMatrix{T}) where {T<:AbstractFloat}
    θ_2 = Vector{T}(undef, size(z, 1))
    for m in axes(z, 1)
        β_2 = μ_k + σ[m,2] * z[m,2]
        θ_2[m] = (1 - θ_cum[m, 1])*sigmoid(β_2)
    end
    return (p0[2] - mean(θ_2))^2
end
