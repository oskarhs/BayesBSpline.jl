    ...
    function BSMModel{T}(x::AbstractVector{<:Real}, basis::A; n_bins::Union{Nothing,<:Integer}=1000, a_τ::Real=1.0, b_τ::Real=1e-3, a_δ::Real=0.5, b_δ::Real=0.5) where {T<:Real, A<:AbstractBSplineBasis}
        ...
        if !isnothing(n_bins)
            ...
            log_B = log.(B)
                ...
                D = BandedMatrix((0=>fill(1, K-3), 1=>fill(-2, K-3), 2=>fill(1, K-3)), (K-3, K-1))
            data = (log_B = log_B, b_ind = b_ind, bincounts = bincounts, n = n)
        else
            B, b_ind = create_spline_basis_matrix(T_x, basis)
            log_B = log.(B)
            data = (log_B = log_B, b_ind = b_ind, n = n)
        end
...
function Base.show(io::IO, ::MIME"text/plain", bsm::BSMModel{T, A, NamedTuple{(:log_B, :b_ind, :bincounts, :n), Vals}}) where {T, A, Vals}
...
function Base.show(io::IO, ::MIME"text/plain", bsm::BSMModel{T, A, NamedTuple{(:log_B, :b_ind, :n), Vals}}) where {T, A, Vals}
    println(io, length(bsm), "-dimensional ", nameof(typeof(bsm)), '{', eltype(bsm), "}:")
