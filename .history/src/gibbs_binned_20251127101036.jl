...
function pointwise_quantiles(θ, t, q)
    S_samp = Matrix{T}(undef, (length(t), size(θ, 2)))
    for i in axes(θ, 2)
        S_samp[:,i] = Spline(basis, BayesBSpline.theta_to_coef(θ[:,i])).(t)
    end
...
lower = pointwise_quantiles(θ, t, 0.05) / (1.1*R)
upper = pointwise_quantiles(θ, t, 0.95) / (1.1*R)
