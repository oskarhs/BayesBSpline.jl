...
"""
struct BSMChains{T<:Real, M<:AbstractBSplineBasis, V<:AbstractVector{<:NamedTuple}}
    samples::V
    model::M
    n_samples::Int
    n_burnin::Int
    function BSMChains(samples::V, model::M, n_samples::Int, n_burnin::Int) where {M, V}
        return new{eltype(eltype(V)), M, V}(samples, model, n_samples, n_burnin)
    end
end
