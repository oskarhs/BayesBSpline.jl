...
function Base.show(io::IO, ::MIME"text/plain", bsm::BSMModel{T, M, NamedTuple{(:B, :b_ind, :bincounts), D}}) where {T, M, D}
    println(io, length(bsm), "-element ", nameof(typeof(bsm)), '{', eltype(bsm), '}', " with binned data:")
    print(io, " basis: ")
...
Base.show(io::IO, bsm::BSMModel{T, M, NamedTuple{(:B, :b_ind, :bincounts), D}}) where {T, M, D} = show(io, MIME("text/plain"), bsm)
