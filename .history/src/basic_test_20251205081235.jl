...
x = rand(rng, d_true, 15000)
bsm = BayesBSpline.BSMModel(x, BSplineBasis(BSplineOrder(4), LinRange(minimum(x), maximum(x), 10001))
...
p = Plots.plot()
Plots.plot!(p, t_orig, S.(t_orig), color=:black, lw=1.2, label="Posterior mean")
