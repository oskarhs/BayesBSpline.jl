...
"""
    ...
    Distributions.mean(bsmc::BSMChains, t::AbstractVector{<:Real}) -> AbstractVector{<:Real}
...
"""
    quantile(bsmc::BSMChains, t::AbstractVector{<:Real}, q::Real) -> AbstractVector{<:Real}
    quantile(θ::AbstractMatrix{T}, basis::A, t::AbstractVector{<:Real}, q::AbstractVector{<:Real}) -> AbstractMatrix{<:Real}

Compute the posterior quantiles of f(t) via Monte Carlo samples.

The latter function returns a Matrix of dimension (length(t), length(q))
"""
function Distributions.quantile(bsmc::BSMChains, t::AbstractVector{T}, q::T) where {T<:Real, A<:AbstractBSplineBasis}
    if !(0 ≤ q ≤ 1)
        throws(DomainError("Requested quantile level is not in [0,1]."))
    end
    S_samp = Matrix{T}(undef, (length(t), size(θ, 2)))
    for i in axes(θ, 2)
        S_samp[:,i] = Spline(basis, bsmc.coef[:,i]).(t)
    end
    #return [quantile(S_samp[j,:], q) for j in eachindex(t)]
    return mapslices(x -> quantile(x, q), S_samp; dims=2)[:]
end

function pointwise_quantile(θ::AbstractMatrix{T}, basis::A, t::AbstractVector{T}, q::AbstractVector{T}) where {T<:Real, A<:AbstractBSplineBasis}
    if !all(0 .≤ q .≤ 1)
