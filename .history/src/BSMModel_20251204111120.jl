...
* `a_δ`: Prior shape hyperparameter for the local smoothing parameters δₖ².
* `b_δ`: Prior rate hyperparameter for the local smoothing parameters δₖ².
...
struct BSMModel{T<:Real, A<:AbstractBSplineBasis, NT<:NamedTuple}
    ...
    a_δ::T
    b_δ::T
    function BSMModel{T,A}(x::AbstractVector{<:Real}, basis::A; n_bins::Union{Nothing,<:Integer}=1000, a_τ::Real=1.0, b_τ::Real=1e-3, a_δ::Real=0.5, b_δ::Real=0.5) where {T<:Real, A<:AbstractBSplineBasis}
