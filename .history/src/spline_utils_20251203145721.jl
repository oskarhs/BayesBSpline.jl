...
Compute the 
"""
struct NormalizedBSplineBasis{A<:AbstractBSplineBasis, T<:Real} <: AbstractBSplineBasis
    basis::A
    norm_fac::Vector{T}

    function NormalizedBSplineBasis(basis::A, ::Type{T}) where {A<:AbstractBSplineBasis, T<:Real}
