...
# Compute the vector Z of normalizing constants for a given B-spline basis.
...
function compute_μ(basis::A, T) where {A<:AbstractBSplineBasis}
    K = length(basis)
...
function compute_μ(basis::A) where {A<:AbstractBSplineBasis}
    return compute_μ(basis, Float64)
end